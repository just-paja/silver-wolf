...
class TimeLimitedModel(TimeStampedModel):
    ...
    end = DateField(blank=True, null=True, verbose_name=_('End'))
...
class TimeFilteredModel(TimeLimitedModel):
    ...
    def clean(self):
        if self.end:
            if self.start >= self.end:
                raise ValidationError({
                    'end':
                    _('End must be blank of greater than start'),
                })
        ...
        if clashes.count() > 0:
            raise ValidationError(
                _('Selected interval clashes with %s' % (clashes[0])))
...
class TimeLimitedManager(Manager):
    ...
    def get_active(self, date=None):
        ...
        return self.filter(Q(end__isnull=True) | Q(end__gte=filter_date),
                           start__lte=filter_date)
